...
bool HasImportableExtensions(const base::FilePath& secured_preference_path) {
  ...
  if (auto* extensions = secured_preference->FindPath("extensions.settings")) {
    auto extensions_list =
        GetImportableExtensionsFromChromeExtensionsList(*extensions);
...
bool ChromeImporterCanImport(const base::FilePath& profile,
  ...
  *services_supported = importer::NONE;
  ...
  base::FilePath secured_preference = profile.Append(
...
std::vector<std::string> GetImportableExtensionsFromChromeExtensionsList(
  ...
  for (const auto& item : extensions_list.DictItems()) {
    ...
    if (auto* manifest_value = item.second.FindDictKey("manifest")) {
      ...
      const auto& manifest = Value::AsDictionaryValue(*manifest_value);
