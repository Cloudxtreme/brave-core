...
class DatabaseTest : public ::testing::Test {
 protected:
  ...
  void CreateTempDatabase(
    ...
    *db_file = temp_dir->GetPath().AppendASCII("DatabaseTest.db");
    sql::Database::Delete(*db_file);

    database = std::make_unique<Database>(*db_file);
    ASSERT_NE(database, nullptr);
    auto callback = std::bind(&DatabaseTest::OnInitialize,
        this,
        _1);
    database->Initialize(false, callback);
  }

  void OnInitialize(const ledger::Result result) {
